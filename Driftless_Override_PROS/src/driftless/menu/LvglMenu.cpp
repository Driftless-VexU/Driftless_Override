...
void settingsBackButtonEventHandler(lv_event_t* event) {
  ...
  if (obj == lv_menu_get_sidebar_header_back_button(menu)) {
    lv_obj_clean(lv_screen_active());
    if (lvgl_menu) {
...
void settingsButtonMatrixEventHandler(lv_event_t* event) {
  ...
  uint32_t button_id{lv_buttonmatrix_get_selected_button(obj)};
...
void LvglMenu::initStyles() {
  ...
  ********************/
  ...
  lv_style_set_shadow_offset_y(&button_pressed_style, 3);
...
void LvglMenu::drawMainMenu() {
  ...
  lv_obj_set_style_bg_color(lv_screen_active(), lv_color_make(173, 205, 234), 0);
  lv_obj_refresh_style(lv_screen_active(), LV_PART_MAIN, LV_STYLE_BG_COLOR);

  // Create the big line at the bottom
  static lv_point_precise_t big_line_points[] = {{0, 205}, {480, 205}};
  ...
  lv_obj_t* big_line = lv_line_create(lv_screen_active());
  ...
  static lv_point_precise_t stripe_line_points[] = {{0, 220}, {480, 220}};
  ...
  lv_obj_t* stripe_line = lv_line_create(lv_screen_active());
  ...
  static lv_point_precise_t left_diagonal_line_points[] = {{320, 190}, {510, 0}};
  ...
  lv_obj_t* left_diagonal_line = lv_line_create(lv_screen_active());
  ...
  static lv_point_precise_t right_diagonal_line_points[] = {{370, 190}, {560, 0}};
  ...
  lv_obj_t* right_diagonal_line = lv_line_create(lv_screen_active());
  ...
  lv_obj_t* team_name_label = lv_label_create(lv_screen_active());
  ...
  lv_obj_t* start_button = lv_button_create(lv_screen_active());
  ...
  lv_obj_t* settings_button = lv_button_create(lv_screen_active());
...
void LvglMenu::drawSettingsMenu() {
  ...
  lv_obj_t* back_button{lv_menu_get_sidebar_header_back_button(menu)};
  lv_obj_remove_style_all(back_button);
  lv_obj_add_style(back_button, &button_default_style, 0);
  lv_obj_add_style(back_button, &button_pressed_style, LV_STATE_PRESSED);
  lv_obj_set_style_text_font(back_button, &lv_font_montserrat_14, 0);
  lv_obj_set_style_text_font(back_button, &lv_font_montserrat_14,
                             LV_STATE_PRESSED);
  lv_obj_set_style_pad_all(back_button, 3, 0);
  lv_obj_set_style_pad_all(back_button, 3, LV_STATE_PRESSED);
  lv_obj_set_style_translate_y(back_button, 0, LV_STATE_PRESSED);
  lv_obj_set_style_shadow_offset_y(back_button, 0, LV_STATE_PRESSED);
  ...
  for (Option& option : options) {
    ...
    lv_obj_t* option_button_matrix{lv_buttonmatrix_create(option_page)};
    lv_buttonmatrix_set_map(option_button_matrix,
                         option_button_matrix_map->data());
    lv_buttonmatrix_set_one_checked(option_button_matrix, true);
    lv_buttonmatrix_set_button_ctrl_all(option_button_matrix,
                                  LV_BUTTONMATRIX_CTRL_CHECKABLE);
    lv_buttonmatrix_set_button_ctrl(option_button_matrix, option.selected,
  ...
  lv_obj_send_event(lv_obj_get_child(
